...
class GameViewModel: ObservableObject {
    ...
    @Published var finalScores: [String: String] = [:]
    ...
    func fetchFinalScore(for game: Game) {
        guard finalScores[game.id] == nil, let sportKey = game.sport_key else { return }
        let urlString = "\(OddsAPI.resultsBaseURL)/\(sportKey)/events/\(game.id)/scores?apiKey=\(OddsAPI.apiKey)"
        ...
        URLSession.shared.dataTask(with: url) { data, response, error in
            if let data = data,
               ...
               scores.count >= 2 {
                let team1 = scores[0]
                let team2 = scores[1]
                let scoreString = "\(team1.name) \(team1.score ?? 0) - \(team2.score ?? 0) \(team2.name)"
                DispatchQueue.main.async {
                    self.finalScores[game.id] = scoreString
                }
            }
        }.resume()
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("\(game.home_team ?? "") vs \(game.away_team ?? "")")
                .font(.system(size: 18, weight: .semibold))
                .foregroundColor(.black)
            Text(gameViewModel.finalScores[game.id] ?? "Loading...")
                .font(.system(size: 16))
                .foregroundColor(.gray)
        }
        .padding()
        ...
        .cornerRadius(15)
        .onAppear {
            gameViewModel.fetchFinalScore(for: game)
        }
    }
}
...
struct ScoresView: View {
    ...
    var completedGames: [Game] {
        gameViewModel.allGames.filter { $0.completed == true }
    }
    ...
    var body: some View {
        ...
        .onAppear {
            gameViewModel.loadAllGames(forceRefresh: true)
        }
    }
}
