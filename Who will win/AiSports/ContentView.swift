...
class GameViewModel: ObservableObject {
    ...
    @Published var completedGames: [CompletedGame] = []
        ...
        for sportKey in supportedSportKeys {
            ...
            URLSession.shared.dataTask(with: url) { data, response, error in
                ...
                do {
                    ...
                    combinedGames.append(contentsOf: games)
                } catch {
    ...
    func loadCompletedGames() {
        ...
        let dispatchGroup = DispatchGroup()
        var loaded: [CompletedGame] = []
        var encounteredError: String? = nil

        for sportKey in supportedSportKeys {
            dispatchGroup.enter()
            let urlString = "\(OddsAPI.resultsBaseURL)/\(sportKey)/scores/?daysFrom=1&apiKey=\(OddsAPI.apiKey)"
            guard let url = URL(string: urlString) else {
                encounteredError = "Invalid URL for sport \(sportKey)"
                dispatchGroup.leave()
                continue
            ...
            URLSession.shared.dataTask(with: url) { data, response, error in
                ...
                guard let data = data else {
                    encounteredError = "No data returned for scores \(sportKey)"
                    return
                }
                do {
                    let games = try JSONDecoder().decode([CompletedGame].self, from: data)
                    loaded.append(contentsOf: games)
                } catch {
                    encounteredError = "Decoding error for \(sportKey): \(error.localizedDescription)"
                }
            }.resume()
        ...
        dispatchGroup.notify(queue: .main) {
            ...
            if let err = encounteredError {
                ...
                self.completedGames = loaded.filter { game in
                    guard let date = game.commenceTimeAsDate else { return false }
                    return game.completed == true && date > cutoff
                }.sorted { ($0.commenceTimeAsDate ?? Date.distantPast) > ($1.commenceTimeAsDate ?? Date.distantPast) }
                self.updateForecastResultsFromCompletedGames()
            }
        }
    }

    private func updateForecastResultsFromCompletedGames() {
        for game in completedGames {
            if let index = ForecastHistoryStore.shared.forecasts.firstIndex(where: { $0.gameID == game.id }) {
                let winner = determineWinner(from: game)
                let actual = "\(winner) wins!"
                ForecastHistoryStore.shared.forecasts[index].actualResult = actual
                ForecastHistoryStore.shared.forecasts[index].isCorrect = ForecastHistoryStore.shared.forecasts[index].result == actual
            }
        }
    }
    ...
    private func determineWinner(from game: CompletedGame) -> String {
        guard let scores = game.scores, scores.count >= 2 else { return "" }
        let team1Score = scores[0].score ?? 0
        let team2Score = scores[1].score ?? 0
        return team1Score > team2Score ? scores[0].name : scores[1].name
    }
...
struct CompletedGame: Identifiable, Decodable {
    ...
    var commenceTimeAsDate: Date? {
        let formatter = ISO8601DateFormatter()
        return formatter.date(from: commence_time)
    }
}
...
struct MainTabView: View {
    var body: some View {
        TabView {
            ...
            .tabItem { Label("Completed", systemImage: "sportscourt") }
            NavigationView {
                SettingsView()
            }
    ...
    var body: some View {
        ZStack {
            ...
            if gameViewModel.isLoading {
                ...
                Text("Error: \(error)")
                    .foregroundColor(.red)
            } else if gameViewModel.completedGames.isEmpty {
                Text("No completed games in last 12 hrs")
                    .foregroundColor(.gray)
            } else {
                ScrollView {
                    LazyVStack(spacing: 12) {
                        ForEach(gameViewModel.completedGames) { game in
                            CompletedGameCardView(game: game)
                        }
                    }
                    .padding()
                }
        ...
        .onAppear { gameViewModel.loadCompletedGames() }
    }
}

struct CompletedGameCardView: View {
    let game: CompletedGame
    @EnvironmentObject var historyStore: ForecastHistoryStore

    var forecast: Forecast? {
        historyStore.forecasts.first { $0.gameID == game.id }
    }
    ...
    func score(for team: String) -> Int {
        game.scores?.first { $0.name == team }?.score ?? 0
    }

    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("\(game.home_team) \(score(for: game.home_team)) - \(score(for: game.away_team)) \(game.away_team)")
                .font(.system(size: 18, weight: .semibold))
                .foregroundColor(.black)
            if let forecast = forecast {
                HStack {
                    Text("Your pick: \(forecast.result)")
                        .font(.system(size: 14))
                        .foregroundColor(.gray)
                    Spacer()
                    let correct = forecast.result.contains(winnerName)
